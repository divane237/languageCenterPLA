  ...
  return (
    <section className="mt-4">
      ...
      <div className="my-2 py-2 min-h-caroussel  [mask-image:linear-gradient(to_left,transparent,#000_20%_80%,transparent)] overflow-hidden">
        {/* Slider  */}
        <ul
          className={`flex w-caroussel relative justify-center items-center h-item bg-gray-200`}
        >
          {/* List */}
          {ImgLinks.map((img, i) => {
            return (
              <li
                className={`absolute right-0 animate-caroussel h-item w-item p-4 backdrop-grayscale`}
                key={img}
                style={{
